...
//     fetch('/quiz/ushow/') // 백엔드에 퀴즈 데이터 요청, 명세서 URL로 설정
//       .then(response => response.json()) // 응답을 JSON 형식으로 변환
//       .then(data => {
//         const quizzes = data.uquiz_list; // 받아온 퀴즈 데이터 목록

//         console.log(data.uquiz_list)
    ...
    .then(data => {
      ...
      function loadCurrentQuiz() {
        if (currentQuizIndex < quizzes.length) {
          ...
          buttonTrue.textContent = 'O';
          buttonTrue.addEventListener('click', () => {
            submitAnswer(quiz.u_quiz_id, quiz.u_answer, true);
            buttonTrue.disabled = true; // O 버튼 비활성화
            buttonFalse.disabled = true; // X 버튼도 비활성화
          });
          answerFormDiv.appendChild(buttonTrue);
          ...
          buttonFalse.textContent = 'X';
          buttonFalse.addEventListener('click', () => {
            submitAnswer(quiz.u_quiz_id, quiz.u_answer, false);
            buttonTrue.disabled = true; // O 버튼 비활성화
            buttonFalse.disabled = true; // X 버튼도 비활성화
          });
          answerFormDiv.appendChild(buttonFalse);
...
function submitAnswer(quizId, correctAnswer, userAnswer) {
  // 제출할 데이터 생성
  const data = {
    quiz_id: quizId,
  ...
  .then(data => {
    ...
    if (userAnswer === correctAnswer) {
      ...
      console.log("오답입니다.");
      showIncorrectAnswer(); // 오답인 경우 오답 메시지와 참고 정보 표시
    }
  })
  .catch(error => {
...
function showIncorrectAnswer() {
  ...
  referenceDiv.textContent = `오답입니다!`;
